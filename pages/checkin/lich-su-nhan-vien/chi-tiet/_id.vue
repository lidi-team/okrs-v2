...
@Component({
  name: 'DetailHistoryPageEmployee',
  head() {
    return {
      title: 'Chi tiết lịch sử Check-in của nhân viên',
    };
  },
  async mounted() {
...
export default class DetailHistoryPageEmployee extends Vue {
  ...
  private goBack() {
    if (this.historyDetail) {
      this.$router.push(`/checkin/lich-su-nhan-vien/${this.historyDetail.objective.id}`);
    }
  }
}
</script>
